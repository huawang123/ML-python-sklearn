...
if __name__ == '__main__':
    ...
    if os.path.exists(model_file):
        ...
        # param = {'max_depth': 3, 'eta': 0.3, 'silent': 1, 'objective': 'multi:softmax', 'num_class': 2}
        # watchlist = [(data_train, 'eval'), (data_test, 'train')]
        # n_round = 7
        # model = xgb.train(param, data_train, num_boost_round=n_round, evals=watchlist)
        #####################  randomforest
        model = RandomForestClassifier(n_estimators=50,max_depth=10,min_samples_split=7,oob_score=True,class_weight={0:1,1:1/y_train.mean()})
        model.fit(x_train, y_train)
